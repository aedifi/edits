...
function HandleCountCommand(a_Split, a_Player)
	...
	if not(State.Selection:IsValid()) then
		a_Player:SendMessage(cChatColor.LightGray .. "Couldn't find any region to count.")
		return true
    end
    
	if (a_Split[2] == nil) then
		a_Player:SendMessage(cChatColor.LightGray .. "Usage: //count <block>")
		return true
	end
	...
	a_Player:SendMessage(cChatColor.LightGray .. "Counted " .. NumBlocks .. " block(s) in total.")
	return true
end
...
function HandleDeselectCommand(a_Split, a_Player)
	...
	a_Player:SendMessage(cChatColor.LightGray .. "Cleared your region.")
	return true
end
...
function HandleDistrCommand(a_Split, a_Player)
	local State = GetPlayerState(a_Player)
	if not(State.Selection:IsValid()) then
		a_Player:SendMessage(cChatColor.LightGray .. "Couldn't find any region to quantify.")
		return true
	end
	...
	a_Player:SendMessage(cChatColor.LightGray .. "Quantified " .. TotalCount .. " block(s) in total.")
...
function HandleExpandContractCommand(a_Split, a_Player)
	local State = GetPlayerState(a_Player)
	if not(State.Selection:IsValid()) then
		a_Player:SendMessage(cChatColor.LightGray .. "Couldn't find any region to lengthen.")
		return true
	end
	...
	if ((a_Split[1] == "//expand") and (a_Split[2] == "vert")) then
		...
		a_Player:SendMessage(cChatColor.LightGray .. "Expanded your region to " .. State.Selection:GetSizeDesc() .. " block(s) in size.")
		return true
	end

	if (a_Split[2] ~= nil) and (tonumber(a_Split[2]) == nil) then
		a_Player:SendMessage(cChatColor.LightGray .. "Usage: " .. a_Split[1] .. " [amount] [direction]")
		return true
	end

	local NumBlocks = a_Split[2] or 1
	...
	a_Player:SendMessage(cChatColor.LightGray .. a_Split[1]:sub(3, -1):ucfirst() .. "ed the selection to " .. State.Selection:GetSizeDesc() .. " block(s) in size.")
	return true
end
...
function HandleHPosCommand(a_Split, a_Player)
	...
	local Success, Msg = State.Selection:SetPos(TargetBlock.x, TargetBlock.y, TargetBlock.z, BlockFace, PointName)
...
function HandlePosCommand(a_Split, a_Player)
	...
	local Success, Msg = State.Selection:SetPos(Pos.x, Pos.y, Pos.z, BLOCK_FACE_TOP, PointName, true)
...
function HandleShiftCommand(a_Split, a_Player)
	local State = GetPlayerState(a_Player)
	if not(State.Selection:IsValid()) then
		a_Player:SendMessage(cChatColor.LightGray .. "Couldn't find any region to shift.")
		return true
	end

	if (a_Split[2] ~= nil) and (tonumber(a_Split[2]) == nil) then
		a_Player:SendMessage(cChatColor.LightGray .. "Usage: //shift [amount] [direction]")
		return true
	end
...
function HandleShrinkCommand(a_Split, a_Player)
	...
	if not(State.Selection:IsValid()) then
		a_Player:SendMessage(cChatColor.LightGray .. "Couldn't find any region to shrink.")
		return true
	end
...
function HandleSizeCommand(a_Split, a_Player)
	local State = GetPlayerState(a_Player)
	if (not State.Selection:IsValid()) then
		a_Player:SendMessage(cChatColor.LightGray .. "Couldn't find any region to size.")
		return true
	end

	a_Player:SendMessage(cChatColor.LightGray .. "Sized your region at " .. State.Selection:GetSizeDesc() .. " block(s) in total.")
	return true
end
